...
Prompt Decorators are case-sensitive `+++Name` controls interpreted by Claude Code as the
active assistant. They are not executable commands and cannot create permissions, tools,
or capabilities.

Claude Code does not natively parse `+++` tokens; there is no built-in decorator engine.
This contract is honored because the FieldOps runtime profile in `CLAUDE.md` is loaded as
project memory at session start, so Claude applies these controls as instructed behavior
rather than through a native feature. The catalog, parameters, and numeric limits below
are authoritative and stay consistent with `CLAUDE.md` and the `fieldops-prompt-decorators`
skill.
...
- Source-related decorators (`CiteSources`, `FactCheck`, `Export`) require credible,
  available evidence; never invent citations, browsing, tool access, or test results.
- Invalid parameters do not activate that decorator and are not silently clamped: report
  the invalid decorator and leave it inactive while other valid decorators still apply.
- For long or ambiguous prompts, run the deterministic, standard-library parser at
  `${CLAUDE_SKILL_DIR}/scripts/parse_decorators.py` (from the `fieldops-prompt-decorators`
  skill) to get a token and parameter report before applying state. It classifies tokens
  only and never executes them.
